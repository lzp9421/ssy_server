...
class knParser{
	...
	protected function toAbsoluteUrl($urlField){
		...
		return $this->url_prefix . $urlBase;
	}
	/** Parser for non-html **/
	...
	function parseHTML(){
		...
		$code = preg_replace('~<\s*html[^>]*>~iUs','<html><meta http-equiv="content-type" content="text/html;charset=gbk">',$code);
		$code = preg_replace('~<input[^>]+?type = button[^>]+>~is','<input type="submit" onClick="Chkradio()">',$code);
